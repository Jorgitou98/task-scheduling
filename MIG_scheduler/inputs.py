...
def get_input_config():
    ...
    return repetitions, n, instance_sizes, perc_scale, perc_membound, device

def generate_tasks(n, instance_sizes, perc_scale, perc_membound, device):
    n_slices = instance_sizes[-1]
    times = []
    for scale_size in instance_sizes:
        n_instance_scale_size = int(n * perc_scale[scale_size] / 100)
        times_instance_scale_size =  [[(1, random.uniform(0.1, 100))] for _ in range(n_instance_scale_size)]
